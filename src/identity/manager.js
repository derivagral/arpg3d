 ...
 * }} opts
 */
export const createIdentityManager = ({ providers, storage = globalThis.localStorage }) => {
  ...
  return {
    ...
    async resolve() {
      ...
      if (provider && !isAnonProvider(provider) && await safeAvailable(provider)) {
        try {
          ...
          if (identity) return set(identity)
        } catch (err) {
          console.warn(`[identity] restore via '${provider.id}' failed:`, err)
        }
    ...
    async signIn(providerId, input, { returnTo } = {}) {
      ...
      writeKey(RETURN_KEY, returnTo ?? currentPath())
    ...
    async completeRedirect(providerId = readKey(SESSION_KEY)) {
      ...
      if (!provider?.completeRedirect) {
        throw new Error('identity: no redirect in progress')
      }
    ...
    async signOut() {
      ...
      if (previous && !isAnon(previous)) {
        ...
        try {
          ...
          console.warn('[identity] sign-out failed (continuing to guest):', err)
        }
      }
      rememberProvider('anon')
      return set(await anon.restore())
